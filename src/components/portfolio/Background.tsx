...
export default function Background() {
  ...
  // Store random particle positions and animation values in state
  const [particleStyles, setParticleStyles] = useState<
    { left: string; top: string; duration: number; delay: number }[]
  >([]);

  useEffect(() => {
    // Generate random styles for each particle only on the client
    const styles = Array.from({ length: particleCount }).map(() => ({
    ...
    // Re-generate if particleCount changes (e.g., on resize)
  }, [particleCount]);
